...
import { CrmApiService, ORDER_STATUSES, OrderResponse, OrderStatus, PageResponse } from '../../core/api';
...
export class OrdersComponent implements OnInit {
  ...
  lineForm={productId:null as number|null,quantity:1,unitPrice:0}; newStatus:OrderStatus='EN_ATTENTE'; lookupId:number|null=null;
  ...
  addLine():void{const o=this.selected(),f=this.lineForm;if(!o||!f.productId)return;this.busy=true;this.api.addProductToOrder(o.idCommande,f.productId,f.quantity,f.unitPrice).subscribe({next:()=>{this.busy=false;this.feedback.success('Product added to order.');this.close();this.load()},error:e=>{this.busy=false;this.feedback.error(e)}})}
