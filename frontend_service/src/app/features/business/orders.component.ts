...
export class OrdersComponent implements OnInit {
  ...
  queueDescription():string{return this.role==='ROLE_CSM'?'Confirm pending orders to hand them to logistics. You only see orders from leads assigned to you.':this.role==='ROLE_LOGISTIC'?'Confirmed orders are ready for logistics. Move them to preparation, then create and assign their shipment in Deliveries.':'Follow each order from customer confirmation through preparation and delivery.'}
  owner(order:OrderResponse):string{return order.statutCommande==='EN_ATTENTE'?'CSM':order.statutCommande==='CONFIRMEE'||order.statutCommande==='PREPARATION'?'Logistics':order.statutCommande==='EXPEDIEE'?'Courier / carrier':'Completed'}
  nextAction(order:OrderResponse):string{return order.statutCommande==='EN_ATTENTE'?'Confirm customer intent':order.statutCommande==='CONFIRMEE'?'Accept for preparation':order.statutCommande==='PREPARATION'?'Create shipment in Deliveries':order.statutCommande==='EXPEDIEE'?'Complete delivery execution':'No operational handoff pending'}
