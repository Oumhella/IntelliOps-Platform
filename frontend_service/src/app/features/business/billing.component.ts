...
export class BillingComponent implements OnInit {
  ...
  download(invoice: InvoiceResponse): void { this.api.downloadInvoice(invoice.id).subscribe({ next: (file) => { const url = URL.createObjectURL(file); const link = document.createElement('a'); link.href = url; link.download = `${invoice.numeroFactureUnique}.pdf`; link.click(); URL.revokeObjectURL(url); this.feedback.success('Invoice download started.'); }, error: (error) => this.feedback.error(error) }); }
